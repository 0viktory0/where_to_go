    ...
    def save_images(self, place, img_urls):
        for order, img_url in enumerate(img_urls):
            ...
            try:
                ...
                self.stderr.write(
                    self.style.ERROR(f'Картинка по адресу {img_url} не найдена'),
                )
    ...
    def handle(self, *args, **options):
        for place_url in options['json_url']:
            try:
                ...
                self.stderr.write(
                    self.style.ERROR(f"Описание локации по адресу {options['json_url']} не найдено"),
                )
                continue
            ...
            try:
                ...
                self.stderr.write(
                    self.style.ERROR(f'Недоступно поле "{exception.args[0]}" '),
                )
                continue
